...
if __name__ == '__main__':
    ...
    for parentfolder in sorted([f.path.replace('\\','/') for f in os.scandir(parentofparents) if f.is_dir()]):
    ...
    #folder = r'X:\Downloads\Further\2009\2009-09-20 Fox Theater, Oakland, CA'
        ...
        #   continue
        for folder in sorted([f.path.replace('\\','/') for f in os.scandir(parentfolder) if f.is_dir()]):
