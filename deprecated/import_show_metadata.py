...
def import_show_folders(concert_folders: list, etree_db: SQLiteEtreeDB, config):
    ...
    try:
        with open(
            "mismatched_folders.txt", "r", encoding="utf-8"
        ) as mismatchlogexisting:
            ...
            try:
                ...
                if not tracknames:
                    ...
                    if None in tracks:
                        ...
                        if tagged:
                            print(
                                f"Sucessfully tagged files in folder {concert_folder}"
                            )
                    ...
                    if None in tracks:
                        missingtitlefiles = ", ".join(
                            [
                                file.name
                                for file in show.folder.musicfiles
                                if file.title is None
                            ]
                        )
                        ...
                        show.db.insert_track_metadata(
                            show.etreerec.id,
                            rows,
                            False,
                            show.etreerec.md5key,
                            debug=True,
                        )
...
def move_folder_if_not_exists(source_folder: str, destination_folder: str):
    ...
    if not os.path.exists(destination_folder):
        try:
            ...
            error_message = (
                f"Error creating destination folder '{destination_folder}': {e}"
            )
            print(error_message)
    ...
    if os.path.exists(destination):
        ...
        try:
            ...
            error_message = (
                f"Error moving '{source_folder}' to '{destination_folder}': {e}"
            )
            print(error_message)
            logging.error(error_message)
            return False
